...
| 5 | dev 运行 | swift run AtlasApp 启动 3s 无崩溃 | 启动成功 | pre-existing FAIL（与本变更无关）— UNUserNotificationCenter `bundleProxyForCurrentProcess is nil`：裸 swift-run 可执行无 .app bundle，权限检查链触发 NSException。已用 git stash 在 HEAD 基线复现同一崩溃，确认非本 CHG 引入。色彩回退分支经 atlasFallbackNSColor 直测锁定（testAtlasColorFallbackPathProducesRenderableColor 测的就是 atlasColor 消费的生产路径函数，锁定 sRGB 分量）；颜色目测留待人工。→ Batch E2 修复后转 PASS，见 #6/#7 |
