...
- **门禁基线**（Batch K 收口）: Packages 487/0（Batch J 437 +50：推荐纯函数 23 + snooze 7 + ledger feed 12 + tone 4 + view-init 4）· Apps 58/0（无回退）· contrast 37 checks ALL PASS（Batch J 文档写 36 实测 37，脚本未改）· Apps build 0 新 warning（1 旧 warning 来自 AtlasCoreAdapters/FileOrganizerScanner.swift:25 b805a57，非 Batch K）· swift run 烟测过 · 新/改源文件 wc -l 协调 ≤350（OverviewFeatureView 337 / CommandColumn 185 / LedgerFeed 223 / Recommendation 361——纯函数 enum 略超 350，沿用 Batch J 纯逻辑文件先例可接受）。
