...
- **日期**: 2026-09-15（**v4**；v1.1 = 2026-06-10 的四视角评审修订：工程可行性 / 规格完整性 / 治理一致性 / UX·可达性·本地化）
- **状态**: v4 修订完成。**隐喻部分已退役**，见下方修订记录
- **v4 修订记录**（2026-09-15，`REQ-copy-plain-language`）:
  - **退役整套文书隐喻**：台账 / 回执 / № / 入账 / 作废 / 足迹 / 证据 —— 全部替换为白话。定稿见 `iterations/REQ-copy-plain-language/terminology-baseline.md`
  - **语域规则反转**：原「文书语气**仅限**台账面」的 containment 规则**失去指称对象**（台账面不复存在），改为**全产品单一语域：白话直陈**
  - **不改动**：配色、字型声部、间距、高程、动效、信息架构骨架 —— 变的是**面向用户的词**，不是视觉系统
  - **设计语言名保留**：`Calm Ledger · 平静台账` 是**内部设计名**，不面向用户；保留以免与既有文档、样张、Backlog 条目脱节
  - 实现规格: `Docs/design/2026-09-15-copy-plain-language.md`。**文案判据以 `Docs/COPY_GUIDELINES.md` 为真相源**，本节只记录设计语言侧的变化
  - **内部标识一律保留**（本文件后续出现的这些**都不是面向用户的词**）：Swift token 名（`ledgerPaper` / `ledgerBorder` / `ledgerRule` / `ledgerInk` / `ledgerSecondary` / `ledgerFont()`）、**面名**（「台账面」= 暖纸底那层壳的**内部代号**）、**声部名**（§1.3 声部③，已改称「记录声部」）、路由 case 名（`AtlasRoute.ledger`）、包名（`AtlasFeaturesHistory`）、键前缀（`ledger.*`）。改这些属于键名重构，**不在本 REQ 范围**
...
- 信任三要素成为**结构元素**：详情面板、恢复点徽记、历史记录时间线在主流程中常驻可见，且全部满足 §6「防噱头回链红线」
...
**「Calm Ledger · 平静台账」** = 平静权威的浅色壳（基底）× 等宽精密的数据层 × 记录式的信任层。

> ⚠️ **v4 修订**：设计语言名中的「Ledger / 台账」是**内部名**。**面向用户的词已全部白话化**（台账→历史记录、回执→结果/编号、№→#、证据→未删除项/分类依据）。下方「三层分工」的括注已同步。

三层分工：**气质柔和**（低饱和青绿、白卡、柔影）、**数据精确**（一切数字与路径用等宽字体）、**信任可见**（记录式纸面、`#` 编号、恢复点徽记、三段式说明）。

### 1.2 色彩 — 双气质表面系统

冷调「工作面」承载操作，暖调「台账面」承载记录与凭证。**暖面使用边界**：仅限历史记录屏、工作模块④结果视图、概览右栏最近记录卡片；其余一律冷面。
...
| `ledgerPaper` | `#FDFCF8`，边框 `ledgerBorder` `#E8E0CF` | 记录纸面（内部 token 名保留） |
| `ledgerRule` | `#D8CFBA`（点状分隔线） | 历史记录条目分隔 |
...
| `ledgerInk` | `#2A2620`；`ledgerSecondary` `#7A7160`(4.69:1) | 记录面文字 |
...
| ③ 记录 | 拉丁/数字: New York；**中文: Songti SC（显式 cascadeList 指定，不依赖系统回退）** | 自定义 `Font`（NSFontDescriptor cascade）封装为 `ledgerFont()` | **仅**记录工件：历史记录页标题、`#` 编号、`ATLAS.` 字标 | 不得用于普通标题/正文；serif 中文最小 13pt 且 bold |

> **v4 修订注**：声部③ 原称「台账声部」，随术语退役改称「记录声部」。**Swift 符号名 `ledgerFont()` 不变** —— 键名/符号名是内部标识，不对用户可见（与 `REQ-copy-plain-language` 的「只改值、不改键」一致）。

> 评审实测：`.fontDesign(.serif)` 下中文回退 STSongti-SC（macOS 15 实测），但该回退是系统 cascade 行为、跨版本不保证——故必须显式指定。若样板间阶段宋体混排观感不达预期，**降级方案**：zh 台账声部改用界面声部 + 字重/字距差异化，serif 仅保留数字与 `ATLAS.` 拉丁工件，编号前缀统一用 `#`。**v4**：原 `№`（U+2116）已退役 —— 该字形在 New York 的覆盖问题（原需 M1 实测）随之消解，`#` 无字形风险。
...
- 记录面：暖纸底 + 1.5px 墨色标题底线 + 点状条目分隔线；不用投影（「纸上印刷」平面感）
...
| `stampIn` | spring(response 0.45, damping 0.62)，scale 1.15→1 + 旋转至 −11° | 执行完成、恢复点建立 |
...
> ⚠️ **v4 修订**：本表的**视觉表达不变**（serif 编号、mono 芯片、三段式、恢复点徽记），**文字已全部白话化**。下表「视觉表达」列的引号内文字为 v4 定稿值。

| 概念 | 视觉表达 | 数据来源（评审修订新增） |
|---|---|---|
| 计划编号 `#N` | serif `#`（双语统一视觉符号）；a11y 读法本地化：zh「计划编号 N」/ en "Plan number N" | **全局历史记录持久序列**：`#N` = 历史记录存储的单调递增序号，扫描产出计划时分配，执行后同号记录；跨模块共享同一序列 |
| 扫描编号 | mono 芯片 `扫描编号 #XXXX · HH:mm`（en: `Scan ID #XXXX`） | #XXXX = 扫描摘要内容哈希前 4 位 hex；**首次扫描前芯片隐藏**；多模块各自持有编号，工具栏芯片显示**当前路由**所属模块的最近编号 |
| 三段式说明 | 为什么安全 / 详情（mono）/ 恢复方案（青框 ⛨） | 现有 Finding 解释字段 + 扫描元数据 |
...
| 历史记录条目 | `#N` + 标题 + mono 数据 + 状态（⛨ 有效 / ✓ 已验证 / 已失效） | 现有 task run / recovery 存储 |
| 行动承诺 | 行动栏承诺文案**由计划实际可恢复状态驱动**：全部可恢复→「⛨ 执行前自动建立恢复点 · 保留 N 天 · 全程有记录」；部分→「⛨ X/Y 项可恢复 · …」；无→不显示 ⛨ 句 | 计划的 recovery 元数据 |

**防噱头回链红线**（同列 §6）：任何编号 / 徽记出现处必须可达对应历史记录详情（点击跳转或悬停 mono 数据），唯一豁免 `ATLAS.` 字标；Toast「已记录 #N」必须可点击跳历史记录。
...
- 侧栏分组：**工作**（概览 · 智能清理 · 文件整理 · 应用 ·〔启动项：contingent on D-010 批准，未批准不入侧栏〕）/ **记录**（历史记录 · 权限）/ 沉底（设置 · 关于）　← **v4**：原「台账」改「历史记录」
- 侧栏项 = 图标 + 名称 + 动态状态副标题（沿用 `AtlasSidebarContext`）；选中态 brand 胶囊；顶部 `ATLAS.` serif 字标
- 工具栏：逐屏过滤搜索沿用（**非全局跨屏搜索**；仅作用于当前屏列表/时间线，扫描/执行阶段中禁用）+ 扫描编号芯片（无编号时隐藏）+ 任务中心铃铛（角标沿用）

### 2.2 命名变更：历史 → 台账 → **历史记录**（v4 已反向撤销）

> ⚠️ **v4 修订（2026-09-15）—— 本节结论已被撤销。**
>
> 本节原名「命名变更：历史 → 台账」。经 `REQ-copy-plain-language` 裁定（术语基线 **P-1** + 裁定位 **R-5**）：
>
> - **面向用户的词撤销**：zh「台账」→ **「历史记录」**；en `Ledger` → **`History`**。判据是「非技术 Mac 用户第一次看到能不能懂」——「台账」是会计/公文词，`Ledger` 同属会计词。**`D-012` 被反向撤销**
> - **内部标识保留**：`AtlasRoute.ledger`（case 名）、包名 `AtlasFeaturesHistory`、键前缀 `ledger.*`、Swift 符号 `ledgerFont()` **全部不变** —— 键名/符号名是内部标识，不对用户可见（与「只改值、不改键」一致）
> - **撤销理由与签字状态**：见 `iterations/REQ-copy-plain-language/terminology-baseline.md` 的 R-5。**该撤销属 CONTRACT 人工确认点，签字前不得被下游当作已定事实引用**
>
> 下方原文保留，仅供追溯当时的决策依据 —— **其结论不再有效**。
...
```
┌ 标题区: 模块名 + 计划 #N 副标题              阶段条 ①扫描✓ ②复核 ③执行 ④结果 ┐
├──────────────────────────┬───────────────────────────┤
│ 列表区（风险分组 + 过滤芯片 + 复选）        │ 详情面板（常驻）                    │
├──────────────────────────┴───────────────────────────┤
...
**④ 更名「结果」**（v4 修订；原「回执」）：
④ = 本次计划的**模块内结果视图**（结果摘要 + 恢复点徽记 + 「在历史记录中查看 →」跳转全局历史记录屏），使用暖纸面卡片；与全局历史记录屏的关系：④ 是单次计划的结果，历史记录屏是全部历史的存档。

> **v4 修订注**：「回执」一词两义，v4 拆开 —— 作**阶段条/面板名**时为「结果」（阶段条读作 扫描 / 复核 / 执行 / **结果**），作**标识符**时为「编号」（`扫描编号 #A1F3`）。此前机械统一译成「编号」会让阶段条读出「扫描 / 复核 / 执行 / 编号」。
...
| executionCompleted | ④ 结果 | 结果视图 | 主操作=完成（回②已只读）/ 新扫描 |
...
- **重新扫描 = 显式动作**：仅在①或②提供「重新扫描」按钮，触发确认（「当前计划 #N 将失效」）；新扫描产出新 #N，旧计划以「已失效」状态入历史记录；菜单命令 Cmd+Shift+R 同走此确认
- **选择状态作用域 = 单一计划内**：同一 #N 内跨阶段往返保留勾选/过滤；#N 变更即清空
- **ViewState 宿主 = `AtlasAppModel`**（按路由 keyed），因 AppShellView 以 `.id(route)` 重建视图，feature 内 `@State` 切屏即丢——阶段/勾选/过滤必须跨路由存活（§7 行为不变红线）

**Undo（评审修订）**：执行完成统一发全局 Toast「已记录 #N · [撤销]」——Toast 容器壳层常驻，跨屏可见；撤销与台账屏「还原全部/逐项还原」指向**同一恢复点**（双入口一份真相）；Toast 存续 8s，超时后历史记录屏仍可还原。智能清理现状 `onUndoExecution: nil // TODO` 在 M3 一并接通。
...
| 屏幕 | 版式与要点 |
|---|---|
| **概览** | 问候 + 状态胶囊行（磁盘/恢复点/权限）→ 「下一步」横幅 → 左**指挥台**（健康环 + 模块入口行）/ 右**最近记录**（最近 3–5 条 #N 条目，暖面卡片，逐条可点跳历史记录） |
