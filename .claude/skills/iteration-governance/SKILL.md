...
3. **Validation Gate**（收尾）：`doctor + verify + 真实验证` 全通过才算过。三条门禁卫生规则：
   ...
   - **新增守卫必须做变异检验，并核对输入面覆盖面** —— 守卫「跑一遍是绿的」不构成验证：先把被检对象改坏，确认守卫**真的变红**，再恢复确认回绿。本仓库两次栽在同一处：`REQ-ux-friction-remediation` 的复审查出 4 条「改坏产品代码仍不失败」的空转守卫（`I-5`/`I-8`/`I-10`/`I-11`），`REQ-copy-plain-language` 又查出覆盖面残缺 + 自证断言 + 解析静默丢行（同类第二次，命中 T3）。逐条问 **「这条规则看不到的载体有哪些？」**（源码字面量 / 注释 / 运行时拼接 / 未解析的行）—— `№` 曾硬编码在 5 处 Swift 字面量里，而门禁只扫两份 `.strings`，报 PASS 而仍在渲染。**活性 ≠ 覆盖**：黑名单是穷举、正则是固定搭配、范围有边界，一个案例一条补丁的规则表注定追不上退化速度；故新守卫一律留「改坏即红」的实测记录，并在收口时派**对抗审查**（任务是证伪，不是复核）去补覆盖缺口。对照：三绿（门禁绿 / 测试绿 / 文档齐）之下，`REQ-copy-plain-language` 仍被对抗审查推翻 9 条收口声明。
...
| 日期 | Agent | 触发 | 教训 | 去向 |
|------|-------|------|------|------|
| 2026-09-15 | `控制器` | **T3**（同类第二次） | 「守卫空转 / 自证」第二次出现。第一次：`REQ-ux-friction-remediation` 复审查出 4 条空转守卫（`I-5`/`I-8`/`I-10`/`I-11`）。第二次：`REQ-copy-plain-language` 终审查出覆盖面残缺（`№` 藏在 Swift 字面量而门禁只扫 `.strings`）、自证断言（用例断言自己的输入）、解析静默丢行。**附带教训**：门禁绿 + 测试绿 + 文档齐的三绿状态下，仍被四路对抗审查推翻 **9 条收口声明** —— 收口声明本身是需要被证伪的对象，不是自证的结论。 | **已合入 L3** → 本文档 Validation Gate 第三条门禁卫生规则；实例档案见 `Docs/debug-issues/2026-09/` 的 issue-1..5 |
